    ...
    <div class="max-w-7xl mx-auto relative z-10 space-y-8 animate-fade-in">
        ...
        <div class="glass rounded-3xl border border-white/5 p-6 md:p-8 shadow-2xl">
          <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
              <div class="relative w-full sm:w-64">
                ...
                <input v-model="searchQuery" type="text" placeholder="Cari proyek..." class="w-full pl-10 pr-4 py-2.5 bg-slate-900/60 border border-slate-700 rounded-xl text-sm text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500/50 transition-all">
              </div>
              <button v-if="canManageGlobal" @click="openCreateModal" class="btn btn-primary rounded-xl text-sm shadow-lg shadow-indigo-500/20 active:scale-95 whitespace-nowrap">
          ...
          <div v-if="!loading" class="mt-6 pt-5 border-t border-white/5 flex flex-wrap gap-6">
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="text-[11px] font-bold text-slate-500 uppercase tracking-wider">{{ plannedCount }} Planned</span>
            </div>
            <div class="flex items-center gap-2">
              <span class="w-2 h-2 rounded-full bg-indigo-400"></span>
              <span class="text-[11px] font-bold text-slate-500 uppercase tracking-wider">{{ activeCount }} On Progress</span>
            </div>
            <div class="flex items-center gap-2">
              <span class="w-2 h-2 rounded-full bg-emerald-500"></span>
              <span class="text-[11px] font-bold text-slate-500 uppercase tracking-wider">{{ completedCount }} Completed</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div v-else-if="filteredProjects.length > 0">
          ...
          >
            <div class="card-body pb-0 flex flex-col flex-1">
              <div class="flex items-start justify-between gap-3 mb-3">
                ...
                <span :class="['flex-shrink-0 px-2.5 py-1 text-[9px] font-black uppercase tracking-widest rounded-lg border', statusConfig[project.status]?.class || statusConfig.planned.class]">
              ...
              <div class="mb-5">
                ...
                <div class="w-full h-1.5 bg-slate-900 rounded-full overflow-hidden">
                  <div class="h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full transition-all duration-1000 ease-out" :style="{ width: project.progress + '%' }"></div>
                </div>
              </div>
            </div>

            <div class="px-6 py-4 border-t border-slate-800/70 flex items-center justify-between gap-2 bg-slate-950/20">
              <button @click="openDetails(project)" class="flex items-center gap-1.5 text-xs font-bold text-indigo-400 hover:text-indigo-300 transition-colors">
                Detail
              ...
              <div v-if="project.my_role_id <= 2 || user?.role_id === 1" class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button @click="openEditModal(project)" class="p-2 text-slate-600 hover:text-amber-400 hover:bg-amber-500/10 rounded-lg transition-all" title="Edit">
                  <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                </button>
                <button v-if="project.my_role_id === 1 || user?.role_id === 1" @click="confirmDelete(project.id)" class="p-2 text-slate-600 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-all" title="Hapus">
        ...
        <div v-if="hasMore" class="mt-10 flex justify-center">
          <button @click="loadMore" :disabled="loading" class="flex items-center gap-3 px-8 py-3 bg-slate-900/60 border border-slate-700 rounded-2xl text-slate-400 hover:text-white hover:border-indigo-500/40 hover:bg-slate-800/60 transition-all shadow-xl disabled:opacity-40 font-bold text-sm uppercase tracking-widest">
        ...
        <div class="modal max-w-lg w-full animate-slide-up">
          ...
          <div class="modal-body">
            <form @submit.prevent="submitProject" class="space-y-5">
              ...
              <div class="form-group">
                <label class="label-field">Status Proyek</label>
                <div class="grid grid-cols-3 gap-2">
                  <button
                    v-for="s in ['planned', 'on_progress', 'completed']"
                    :key="s"
                    type="button"
                    ...
                    :class="[
                      ...
                      formData.status === s
                        ? statusConfig[s].class + ' ring-1 ring-offset-1 ring-offset-slate-900 ring-current'
      ...
      <div v-if="showDetailsModal" class="modal-overlay" @click.self="closeDetailsModal">
        <div class="relative bg-slate-900 border border-slate-700/60 rounded-3xl shadow-2xl w-full max-w-4xl max-h-[88vh] flex flex-col overflow-hidden animate-slide-up">
          ...
          <div class="p-7 border-b border-slate-800 flex items-start justify-between gap-6 bg-gradient-to-r from-indigo-950/40 to-slate-900/40">
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span :class="['px-2.5 py-0.5 text-[9px] font-black uppercase tracking-widest rounded border', statusConfig[selectedProject?.status]?.class || statusConfig.planned.class]">
                  {{ statusConfig[selectedProject?.status]?.label || 'Planned' }}
                </span>
              </div>
              ...
              <div class="mt-4 flex items-center gap-3">
                <div class="flex-1 h-1.5 bg-slate-800 rounded-full overflow-hidden max-w-xs">
                  <div class="h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full transition-all duration-700" :style="{ width: (selectedProject?.progress || 0) + '%' }"></div>
                </div>
                <span class="text-[11px] font-black text-indigo-400 tabular-nums">{{ selectedProject?.progress || 0 }}%</span>
              </div>
              <button @click="goToWorkspace(selectedProject.id)" class="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl transition-all shadow-lg shadow-indigo-900/40 active:scale-95">
                Buka Workspace
          ...
          <div class="flex border-b border-slate-800 px-7 bg-slate-900/80">
            <button @click="activeTab = 'tasks'" :class="['py-4 text-[10px] font-black uppercase tracking-widest border-b-2 mr-8 transition-all', activeTab === 'tasks' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-300']">
              Daftar Tugas
            ...
            <button @click="activeTab = 'members'" :class="['py-4 text-[10px] font-black uppercase tracking-widest border-b-2 transition-all', activeTab === 'members' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-300']">
              Anggota Tim
          ...
          <div class="overflow-y-auto flex-1 p-7 detail-scroll bg-slate-950/30">
            ...
            <div v-if="activeTab === 'tasks'" class="animate-fade-in">
              ...
              <div v-else class="space-y-3">
                <div v-for="task in projectTasks" :key="task.id" class="group flex items-center justify-between p-4 bg-slate-900/50 border border-slate-800 rounded-2xl hover:border-slate-700 hover:bg-slate-900/80 transition-all">
                  <div class="flex items-center gap-3 min-w-0">
                    <div :class="['w-2 h-2 rounded-full flex-shrink-0', task.status === 'done' ? 'bg-emerald-500' : task.status === 'doing' ? 'bg-indigo-500' : task.status === 'review' ? 'bg-amber-500' : 'bg-slate-600']"></div>
                    <div class="min-w-0">
                      <p class="text-white font-bold text-sm m-0 leading-tight truncate group-hover:text-indigo-300 transition-colors">{{ task.name || task.title }}</p>
                      <p class="text-[10px] text-slate-500 m-0 mt-0.5">{{ task.user?.name ? `Assigned: ${task.user.name}` : 'Unassigned' }}</p>
                    </div>
                  </div>
                  <span :class="['flex-shrink-0 badge text-[9px]', task.status === 'done' ? 'badge-success' : task.status === 'doing' ? 'badge-primary' : task.status === 'review' ? 'badge-warning' : 'badge-danger']">
                    {{ task.status || 'todo' }}
                  </span>
                </div>
              </div>
